...
export default function Onboarding() {
  ...
  const leads = parseFloat(answers.leads_month || "100");
  const ticket = parseFloat(answers.avg_ticket || "5000");

  const maturityScore = Math.min(100, Math.round(convRate * 4 + (leads > 50 ? 20 : 10) + (revenue > 30000 ? 20 : 10)));
  const potentialRevenue = leads * (convRate * 2 / 100) * ticket;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Diagnóstico de maturidade, GAP de receita e projeção de ROI
            </p>
          </div>
        </div>
        ...
        {!showResults ? (
            ...
            <Card className="border-border/30 bg-card/80">
              <CardHeader className="pb-3">
                ...
                <Progress
                  ...
                  className="h-1.5 [&>div]:bg-primary mt-2"
                />
              </CardHeader>
              <CardContent className="space-y-5">
                {questions.map((q) => (
                  <div key={q.id} className="space-y-2">
                    ...
                    <Input
                      ...
                      className="h-9 bg-background/50 border-border/30"
                    />
                  </div>
                ))}
                <Button
                ...
                >
                  <Calculator className="w-4 h-4 mr-2" /> Calcular Diagnóstico
                </Button>
              </CardContent>
            </Card>
          </motion.div>
          ...
          >
              ...
              <Card className="border-border/30 bg-card/80 metric-card">
                <CardContent className="p-5 text-center">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">
                    {maturityScore < 40 ? "Iniciante" : maturityScore < 70 ? "Em Desenvolvimento" : "Avançado"}
                  </p>
                </CardContent>
              </Card>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground leading-relaxed">
                      ...
                      <span className="text-primary font-semibold">
                        R$ {(revenueGap / 1000).toFixed(0)}K/mês
                      </span>{" "}
                      com a implementação do Método Vértice. Sua operação está no nível{" "}
                      <span className="font-semibold">
                        {maturityScore < 40 ? "Iniciante" : maturityScore < 70 ? "Em Desenvolvimento" : "Avançado"}
                      </span>{" "}
                      e o ROI projetado é de{" "}
                      <span className="text-brand-success font-semibold">{projectedRoi}%</span>.
                    </p>
                  </div>
                </div>
              </CardContent>
            ...
            <div className="flex gap-3">
              <Button variant="outline" onClick={() => setShowResults(false)} className="border-border/30">
                Refazer Diagnóstico
              </Button>
